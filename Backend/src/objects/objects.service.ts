...
export class ObjectsService {
  ...
  async create(createObjectDto: CreateObjectDto, file: any): Promise<ObjectResponseDto> {
    try {
      ...
      console.error('❌ Error creating object:', message);
      throw new InternalServerErrorException(`Failed to create object: ${message}`);
    }
  }
  ...
  async findAll(): Promise<ObjectResponseDto[]> {
    try {
      ...
      const message = error?.message ?? 'Unknown error';
      console.error('❌ Error fetching objects:', message);
  ...
  async findOne(id: string): Promise<ObjectResponseDto> {
    try {
      ...
      const message = error?.message ?? 'Unknown error';
      console.error('❌ Error fetching object:', message);
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      const message = error?.message ?? 'Unknown error';
      console.error('❌ Error deleting object:', message);
